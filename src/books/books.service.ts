...
export class BooksService {
    ...
    async uploadBook(
        ...
        const newBook: Book = {
            ...
            book_pdf:
                files.book_pdf[0].destination + files.book_pdf[0].originalname,
            book_profile:
                files.book_profile[0].destination +
                files.book_profile[0].originalname,
        };
    ...
    async updateBookById(
        ...
        if (
            book_pdf &&
            book_pdf[0].destination + book_pdf[0].originalname !==
                foundBook.book_pdf
        ) {
    ...
    async removeBookById(id: number): Promise<apiResponse> {
        ...
        fs.unlink(foundBook.book_pdf, (err) =>
            err ? console.error(err) : null,
        );
